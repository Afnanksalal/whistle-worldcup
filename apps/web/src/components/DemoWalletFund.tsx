...
export function DemoWalletFund() {
  ...
  return (
    <div className={`demo-wallet-fund${note ? " has-note" : ""}`}>
      <button
        type="button"
        ...
        aria-label={busy ? "Funding demo wallet" : note ? note : "Get demo USDC"}
      >
        {busy ? (
          "Funding…"
        ) : (
      ...
      {note && (
        <span className="demo-wallet-fund-note" role="status" aria-live="polite">
          {note}
        </span>
      )}
    </div>
