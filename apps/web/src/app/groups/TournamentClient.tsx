...
export default function TournamentClient({
  ...
  const [groups, setGroups] = useState<GroupTable[]>(initialData?.groups ?? []);
  const [fixtures, setFixtures] = useState<Fixture[]>(initialData?.fixtures ?? []);
  const [activeGroup, setActiveGroup] = useState<string | null>(
    initialData?.groups[0]?.group ?? null
  );
    ...
    Promise.all([
      ...
      .then(([groupResponse, fixtureResponse]) => {
        ...
        setActiveGroup((current) =>
          current && groupResponse.groups.some((group) => group.group === current)
            ? current
            : groupResponse.groups[0]?.group ?? null
        );
        setError(false);
      })
      .catch(() => {
      ...
      <div className="shell tournament-shell">
        ...
        {loading && (
          <div className="tournament-loading">
            <FootballLoader label="Building the tournament road…" />
          </div>
        )}

        {(!loading || groups.length > 0) && groups.length > 0 && currentGroup && (
