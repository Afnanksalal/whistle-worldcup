...
function solflareProviderFromWindow(): InjectedSolanaProvider | null {
  if (typeof window === "undefined") return null;
  const w = window as Window & {
    solflare?: InjectedSolanaProvider & { isSolflare?: boolean };
    SolflareApp?: InjectedSolanaProvider | unknown;
  };
  if (w.solflare && typeof w.solflare.connect === "function") {
    return w.solflare;
  }
  // Some Solflare in-app browsers expose SolflareApp as the signer, not window.solflare.
  const app = w.SolflareApp;
  if (
    app &&
    typeof app === "object" &&
    typeof (app as InjectedSolanaProvider).connect === "function"
  ) {
    return app as InjectedSolanaProvider;
  }
  return null;
}
...
export function createSolflareBrowseAdapter(): BrowseWalletAdapter {
  return new BrowseWalletAdapter({
    ...
    isInjected: () => Boolean(solflareProviderFromWindow()),
    getProvider: () => solflareProviderFromWindow(),
  });
}
